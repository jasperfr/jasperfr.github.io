...
class Generator {
    ...
    buyGenerator() {
        ...
        $($(this.element).find('.label-value')).text(`価値 x${this.upgrades.value.increment}`);
        $($(this.element).find('.label-speed')).text(`速度 x${this.upgrades.speed.increment}`);
        $($(this.element).find('.label-ascend')).text(`上昇 x${this.upgrades.ascend.increment}`);
    }
    ...
    buyValueUpgrade() {
        ...
        // Set HTML
        $($(this.element).find('.output')).text(`${toByteCode(this.upgrades.value.amount)}を生成。`);
        $($(this.element).find('.cost-value')).text(toByteCode(this.upgrades.value.price));
    }
